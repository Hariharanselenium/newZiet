...
public class SERVER {
	...
	public void the_utc_timing_is_adjusted() throws InterruptedException, JSchException, IOException {
		...
		server.commandinserver("sudo timedatectl set-ntp on");
		server.commandinserver("sudo timedatectl set-timezone UTC");
		server.commandinserver("sudo ntpdate -u pool.ntp.org");
		Thread.sleep(10000);
//		server.commandinserver("echo '" + FrameworkConstants.SERVER_PASSWORD + "' | sudo -S timedatectl set-ntp 1");
//		server.commandinserver("echo '" + FrameworkConstants.SERVER_PASSWORD + "' | sudo -S timedatectl set-timezone UTC");
//		server.commandinserver("echo '" + FrameworkConstants.SERVER_PASSWORD + "' | sudo -S ntpdate -u pool.ntp.org");
